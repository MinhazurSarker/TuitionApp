...
function Posts({ token, postsRes, }) {
    ...
    const filter = (e) => {
        router.push(`/posts/?page=${current}&div=${divission}&dis=${district}&upo=${upozilla}&uni=${union}&search=${search}`)
        setShowFilter(!showFilter);
    }
    ...
    const clearFilter = async (e) => {
        ...
        setSearch('')
        setShowFilter(!showFilter);
        router.push(`/posts/?page=${''}&div=${''}&dis=${''}&upo=${''}&uni=${''}&search=${search}`)
    }
    const clearSearch = (e) => {
        setSearch('')
        router.push(`/posts/?page=${current}&div=${divission}&dis=${district}&upo=${upozilla}&uni=${union}&search=${''}`)
    }
    const prevPage = (e) => {
        if (current > 1) {
            setCurrent(current - 1)
            router.push(`/posts/?page=${current - 1}&div=${divission}&dis=${district}&upo=${upozilla}&uni=${union}&search=${''}`)
        } else {
            Swal.fire({
                icon: 'info',
                title: 'Not available',
            ...
            <div className="2xl:basis-9/12 xl:basis-9/12 lg:basis-9/12 md:basis-full sm:basis-full basis-full h-full lg:mb-8 md:mb-6 sm:mb-4 mb-2">
                <div className="max-w-screen-xl px-4  md:px-8 mx-auto">
                    <div className="flex justify-between items-center gap-4 mb-6">
                        <h2 className="text-gray-800 dark:text-gray-200 text-2xl lg:text-3xl font-bold">All posts</h2>
...
export async function getServerSideProps(ctx) {
    ...
    const res = await fetch(`${process.env.API_URL}/posts/?page=${ctx.query.page || 1}&div=${ctx.query.div || ''}&dis=${ctx.query.dis || ''}&upo=${ctx.query.upo || ''}&uni=${ctx.query.uni || ''}&search=${ctx.query.search || ''}`)
