...
function Followings({ token, tutorsArray }) {
    ...
    const HandleSearch = (e) => {
        e.preventDefault();
        router.push(`/followings?page=${current}&search=${search}`)
    }


    const clearSearch = (e) => {
        setSearch('')
        router.push(`/followings?page=${current}&search=${''}`)
    }
    const prevPage = (e) => {
        if (current > 1) {
            setCurrent(current - 1)
            router.push(`/followings?page=${current - 1}&search=${''}`)
        } else {
            Swal.fire({
                icon: 'info',
                title: 'Not available',
    ...
    return (
        <div className='flex 2xl:flex-row  xl:flex-row  lg:flex-row md:flex-col sm:flex-col flex-col '>
            <head>
                <title>TuitionApp - Followed tutors</title>
            </head>
...
export async function getServerSideProps(ctx) {
    ...
    const res = await fetch(`${process.env.API_URL}/users/followings?page=${ctx.query.page || 1}&search=${ctx.query.search || ''}`, {
        headers: {
            token: token
        }
    })
